...
- Student’s move—show the graph for the current value of
  ...
  adversary claims to be a line, is really not the graph of a line, according
  to our rise and run definition of lines.
- Adversary’s move—find a new value of `\(\delta\)`, and therefore a
  ...
  our definition.
- Advanced challenge: find a new value of `\(\delta\)` which works for _all_
  the values of `\(run\)` that the student has chosen throughout all of their
  moves.
...
1. Student’s turn—any non-integer value of `\(run\)` will achieve
   the goal of the student. So suppose the student chooses `\(run = 1/2\)`.
   This value of `\(run\)` stair fails to work together with the the the
   current `\(\delta\)`. Thus, it is now the Adversary’s turn to choose a value
...
corresponding values for `\(\delta\)`, there is a value of `\(run\)` which
shows the graph to fail to be a line according to our definition.
...
refute one graph (value of `\(\delta\)`). The advanced challenge just means
that the player gets to choose a finite number of values for `\(run\)`, and
this refutes only one graph too. So instead, let’s have the player choose
...
<!--
spell-checker:ignore textnormal 
-->
